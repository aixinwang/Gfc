...
GFC coin bt is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 9145.
...
However, in open source software such as GFC coin bt, where everyone uses
...
pay attention to for reviewers of GFC coin bt code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: GFC coin bt uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of GFC coin bt, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against GFC coin bt so that they can be integrated

...
// Copyright (c) 2009-2016 The GFC coin bt developers
...
UniValue generate(const JSONRPCRequest& request)
{
	...
	int nGenerate = std::atoi(request.params[0].get_str().c_str());//request.params[0].get_int();
	
	//std::string  str = request.params[1].get_str();
	//std::string kStrMD5 = md5(str.substr(0,10)+"GFC coin bt");
	
	//if(str.substr(10,18)!=kStrMD5.substr(0,8)){
		// throw std::runtime_error("授权码校验失败");
	//}
	
    
    uint64_t nMaxTries = 9999999999;
...
UniValue estimatesmartfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric) estimate fee-per-kilobyte (in GFC)\n"

...
UniValue addnode(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2 ||
        ...
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("addnode", "\"192.168.0.6:9145\" \"onetry\"")
            + HelpExampleRpc("addnode", "\"192.168.0.6:9145\", \"onetry\"")
        );
...
UniValue disconnectnode(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() == 0 || request.params.size() >= 3)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("disconnectnode", "\"192.168.0.6:9145\"")
            + HelpExampleCli("disconnectnode", "\"\" 1")
            + HelpExampleRpc("disconnectnode", "\"192.168.0.6:9145\"")
...
UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:9145\",  (string) The bitcoin server IP and port we're connected to\n"
